...
\subsection{Integrability}
  ...
  The following is the proof for (i) of the \emph{FTC,} which comes down to a clever application of telescoping sums.

  \begin{proof}[\textbf{Proof of (i)}]
    Suppose \(f:[a,b] \to \R\) is integrable, and \(F:[a,b] :\R\) satisfies \(F'(x) = f(x)\) for all \(x \in [a,b].\) Let \(P = \{x_0,\ldots, x_n\}\) be an arbitrary partition of \([a,b]\). Narrowing down our focus to a single sub-interval \([x_{i-1},x_i]\), since \(F\) is differentiable, by the Mean Value Theorem, there exists a \(c_i \in [x_{i-1},x_i]\) such that
    \[
